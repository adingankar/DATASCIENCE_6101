...
There are 3 numerical, 17 categorical variables.
There are only 11 missing data in the TotalCharges column. We set them to 0 as the Telecom company has not received payment from these customers.
...
loadPkg("ggplot2")
  ...
  ggplot(aes(Churn, Percent)) + 
    ...
    theme_minimal()
```
Customer churn percentage is 26.54%
...
```{r, results="markup"}
loadPkg("corrplot")
numeric.var <- sapply(telco_data, is.numeric)
corr.matrix <- cor(telco_data[,numeric.var])
